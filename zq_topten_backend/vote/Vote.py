...
import json
from .settings import IP_whiteList,IP_NUM,IP_TOTAL,DEVICE_NUM,DEVICE_TOTAL,START_DATE
from .models import IP,Device,Member,Voter,IllegalVote
from datetime import date
...
def Vote(ip,device,stu,ua,FingerPrint,candidates):
    # 更新IP
    ip.num += 1
    ip.total += 1
    ip.date = date.today()
